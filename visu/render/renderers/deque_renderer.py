...
class DequeRenderer(StructureRenderer):
    kind = "deque"
    ...
    def draw(self, cr, structure, snapshot: Snapshot, poses, scale: float):
        ...
        first = poses.get(self.key(structure, items[0].id))
        last = poses.get(self.key(structure, items[-1].id))

        if first is not None:
            draw_text(
                cr, "front",
                first.x - bw * 0.35,
                first.y - bh / 2 - 6 * scale,
        ...
        if first is not None and last is not None and last.x - first.x > 60 * scale:
            y_arrow = first.y + bh / 2 + 12 * scale
            draw_arrow(
                cr,
                first.x - bw / 2 - 2 * scale, y_arrow,
                first.x - bw / 2 - 16 * scale, y_arrow,
                (*t.muted[:3], 0.55),
            ...
            draw_arrow(
                cr,
                last.x + bw / 2 + 2 * scale, y_arrow,
                last.x + bw / 2 + 16 * scale, y_arrow,
                (*t.muted[:3], 0.55),
                head=max(3.0, 5.0 * scale),
                width=max(0.8, 1.0 * scale),
            )
